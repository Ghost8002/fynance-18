...
const features = ["Transações ilimitadas", "Cartões de crédito ilimitados", "Contas bancárias ilimitadas", "Metas financeiras", "Orçamentos por categoria", "Relatórios detalhados", "Assistente de IA", "Importação de extratos", "Calendário financeiro", "Suporte prioritário"];
export default function PricingPage() {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 py-16 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <Card className="max-w-md mx-auto border-primary/20 shadow-xl shadow-primary/5">
          <CardHeader className="text-center pb-8 pt-8">
            {isSubscribed && <div className="absolute top-4 right-4">
          ...
          <CardContent className="pb-8">
            ...
            {isSubscribed && subscriptionEnd && <div className="mt-6 p-4 rounded-lg bg-muted/50 border">
                <p className="text-sm text-muted-foreground">
                  Sua assinatura renova em{" "}
                  <span className="font-medium text-foreground">
                    {format(new Date(subscriptionEnd), "dd 'de' MMMM 'de' yyyy", {
                  locale: ptBR
                })}
                  </span>
                </p>
          ...
          <CardFooter className="flex flex-col gap-4 pb-8">
            {isSubscribed ? <Button onClick={handleManageSubscription} variant="outline" className="w-full" disabled={checkoutLoading || isLoading}>
                {checkoutLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <CreditCard className="h-4 w-4 mr-2" />}
                Gerenciar assinatura
              </Button> : <Button onClick={handleSubscribe} className="w-full h-12 text-lg" disabled={checkoutLoading || isLoading}>
