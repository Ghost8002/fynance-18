...
const CategorySettings = () => {
  ...
  const { data: categories, loading, insert, update, remove, refetch } = useSupabaseData('categories', user?.id);
  ...
  const handleSaveEdit = async (categoryId: string) => {
    ...
    const { error } = await update(categoryId, {
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        <AlertDescription>
          Categorias padrão não podem ser excluídas, mas podem ser editadas. 
          Categorias personalizadas podem ser completamente gerenciadas por você.
        </AlertDescription>
      </Alert>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  id="category-name"
                  placeholder="Ex: Freelance, Compras, etc."
